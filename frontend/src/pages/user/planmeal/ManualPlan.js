...
const ManualPlan = () => {
    ...
    const handleRecipeClickForMeal = (recipe) => {
        // Update the selected meal's input value with the clicked recipe
        setCreatePlan((prevPlan) => ({
            ...prevPlan,
    ...
    // Function to handle focusing on the input for a specific meal
    const handleMealInputFocus = (meal) => {
        // Set the selected meal when the input is focused
    ...
    const handleCreatePlanMeal = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`${apiUrl}/mealplan/createMealPlan`, createPlan);
                                    ...
                                    {['breakfast', 'lunch', 'dinner', 'snack1', 'snack2'].map((meal) => (
                                        <div key={meal} className={cx('show-breakfast')}>
                                            ...
                                            <div className={cx('recipes')}>
                                                {/* Use the handleMealInputFocus function when the input is focused */}
                                                <input
                                                    type='text'
                                                    placeholder={`recipe for ${meal} ...`}
                                                    onFocus={() => handleMealInputFocus(meal)}
                                                    value={createPlan[meal] || ''}
                                                />
                                                {/* Show the search results when the meal is selected */}
                                                {showResult &&
                                                                ...
                                                                <ul className={cx('show-search')}>
                                                                    {filteredRecipes.map((recipe) => (
                                                                        <li
                                                                            ...
                                                                            key={recipe.id}
                                                                            onClick={() =>
                                                                                handleRecipeClickForMeal(recipe)
                                                                            }
                                                                        >
                                                                            <div className={cx('show-recipe')}>
