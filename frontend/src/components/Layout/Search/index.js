...
function Search() {
    ...
    console.log("Result or History exists:", searchResult.length > 0 || Object.keys(listHistory).length > 0);
    console.log("Search Result:", searchResult);
    console.log("List History:", listHistory);

    return (
        <>
            <form ref={formRef} className="your-form-class">
                <Tippy
                    interactive
                    placement='bottom-start'
                    visible={isSearchInputFocused && (searchResult.length > 0 || Object.keys(listHistory).length > 0)}
                    // visible={isSearchInputFocused && (searchResult.article?.length > 0 || searchResult.recipes?.length > 0 || searchResult.hashtags?.length > 0 || searchResult.users?.length > 0)}
                        ...
                        <div className={cx('search-result')} tabIndex="1"{...attrs}>
                            <PopperWrapper style={{ marginLeft: "0px" }}>
                                {searchResult.article && (
                                    <>
                                        <h5></h5>
                                        {searchResult.article.slice(0, MAX_RESULTS_PER_CATEGORY).map((article) => (
                                ...
                                {searchResult.recipes && (
                                    <>
                                        ...
                                        {searchResult.recipes.slice(0, MAX_RESULTS_PER_CATEGORY).map((recipe) => (
                                            <div className={cx('show-history')} >
                                                <Link to={`/detail/${recipe._id}`} key={recipe._id} >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
                                                        <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" />
                                                    </svg> &nbsp;
                                                    {recipe.name}
                                ...
                                {listHistory.trend && (
                                    <>
                                        {/* Hiển thị hot trend */}
                                        {/* <h5>Hot Trend</h5> */}
                                        {listHistory.trend.slice(0, MAX_HISTORY_ITEMS).map((item) => (
                                ...
                                {listHistory.hashtag && (
                                    <>
                                        {listHistory.hashtag.slice(0, MAX_HISTORY_ITEMS).map((item) => (
                                            <div className={cx('show-history', 'font-text')} key={item._id}>
                                                <svg style={{ color: "red" }} xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-fire" viewBox="0 0 16 16">
                                                    <path d="M8 16c3.314 0 6-2 6-5.5 0-1.5-.5-4-2.5-6 .25 1.5-1.25 2-1.25 2C11 4 9 .5 6 0c.357 2 .5 4-2 6-1.25 1-2 2.729-2 4.5C2 14 4.686 16 8 16Zm0-1c-1.657 0-3-1-3-2.75 0-.75.25-2 1.25-3C6.125 10 7 10.5 7 10.5c-.375-1.25.5-3.25 2-3.5-.179 1-.25 2 1 3 .625.5 1 1.364 1 2.25C11 14 9.657 15 8 15Z" />
                                                </svg> &nbsp;{item.keyword}
                ...
                >
                    <div className="input-group rounded">
                        <input type="search" className={cx("form-control", "rounded")} value={keyword}
                            ...
                            }} placeholder="Search" aria-label="Search" aria-describedby="search-addon" style={{ width: "250px" }} />
